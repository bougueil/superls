defmodule Superls.CLI do
  ...
  defp main_args(["inspect"], store_name, passwd) do
    ...
    100 most frequent tags: #{tags_to_string(info.most_frequent_100)}
  ...
  defp main_args(["help"], _store, _passwd) do
    ...
    Usage: superls command [params] [-p -s my_store]
      -s specify a store name other than default one `default`
      -p password protect the store
  ...
  defp tags_to_string(frequencies) do
    Map.to_list(frequencies)
    |> Enum.sort(&(elem(&1, 1) >= elem(&2, 1)))
    |> Enum.map_join(", ", fn {t, num} -> "#{IO.ANSI.bright()}#{t}#{IO.ANSI.reset()} #{num}" end)
  end
